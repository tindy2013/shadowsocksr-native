...
#endif
static void udp_remote_close_and_free(struct udp_remote_ctx_t *ctx);
...
static int
udprealy_parse_header(const char *buf, const size_t buf_len,
...
void
udp_remote_close_and_free(struct udp_remote_ctx_t *ctx)
{
    if (ctx != NULL) {
    ...
    if (addr == NULL) {
        ...
        if (remote_ctx != NULL) {
            ...
            if (s == -1) {
                ...
                if (!cache_hit) {
                    udp_remote_close_and_free(EV_A_ remote_ctx);
                }
            } else {
                if (!cache_hit) {
                    // Add to conn cache
...
udp_remote_recv_cb(uv_udp_t* handle, ssize_t nread, const uv_buf_t* buf0, const struct sockaddr* addr, unsigned flags)
{
    ...
    if (server_ctx == NULL) {
        ...
        udp_remote_close_and_free(remote_ctx);
        return;
    }
    ...
    int len = udprealy_parse_header(buf->buffer, buf->len, NULL, NULL, &dst_addr);
...
#else
    int len = udprealy_parse_header(buf->buffer, buf->len, NULL, NULL, NULL);
#endif
...
udp_listener_recv_cb(uv_udp_t* handle, ssize_t nread, const uv_buf_t* buf0, const struct sockaddr* addr, unsigned flags)
{
    ...
    if (server_ctx->tunnel_addr.host && server_ctx->tunnel_addr.port) {
        ...
        addr_header_len = udprealy_parse_header(buf->buffer + offset, buf->len - offset,
                                                    host, port, &dst_addr);
        if (addr_header_len == 0) {
            // error in parse header
            goto CLEAN_UP;
    ...
    */
    ...
    if (remote_ctx != NULL) {
        ...
        if (dst_addr.ss_family == AF_INET || dst_addr.ss_family == AF_INET6) {
            int remotefd = udp_create_remote_socket(dst_addr.ss_family == AF_INET6);
            if (remotefd != -1) {
                setnonblocking(remotefd);
#ifdef SO_BROADCAST
                set_broadcast(remotefd);
