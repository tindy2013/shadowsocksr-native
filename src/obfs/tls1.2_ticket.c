...
struct buffer_t * tls12_ticket_auth_client_encode(struct obfs_t *obfs, const struct buffer_t *buf) {
    ...
    if ((local->handshake_status & 4) == 4) {
        ...
        while (local->send_id <=4 && datalength - start > 256) {
            ...
            buffer_concatenate2(result, tmp); buffer_free(tmp);
            start += len;
        }
...
#define CSTR_DECL(name, len, str) const char* (name) = (str); const size_t (len) = (sizeof(str) - 1)
        ...
        size_t pos;
        struct buffer_t *sni;
        struct buffer_t *ext_buf = buffer_alloc(SSR_BUFF_SIZE);
        uint16_t temp = 0;
...
struct buffer_t * tls12_ticket_auth_client_decode(struct obfs_t *obfs, const struct buffer_t *buf, bool *needsendback) {
    struct buffer_t *result = buffer_alloc(SSR_BUFF_SIZE);
    struct tls12_ticket_auth_local_data *local = (struct tls12_ticket_auth_local_data*)obfs->l_data;
    struct tls12_ticket_auth_global_data *global = (struct tls12_ticket_auth_global_data*)obfs->server.g_data;
...
struct buffer_t * tls12_ticket_auth_client_encode(struct obfs_t *obfs, const struct buffer_t *buf) {
    ...
    if (local->handshake_status == 8) {
        if (buf->len < (SSR_BUFF_SIZE / 2)) {
            ...
            result = buffer_create_from(out_buffer, outlength);
            free(out_buffer);
            return result;
        }
    }
...
#define CSTR_DECL(name, len, str) const char* (name) = (str); const size_t (len) = (sizeof(str) - 1)
    ...
    result = buffer_create_from(out_buffer, datalength);
    free(out_buffer);
    return result;
}
...
struct buffer_t * tls12_ticket_auth_client_decode(struct obfs_t *obfs, const struct buffer_t *buf, bool *needsendback) {
    ...
    *needsendback = false;
    ...
    }
    {
        ...
        BUFFER_CONSTANT_INSTANCE(pTotal, encryptdata, buf->len - 10);
        ...
        if (memcmp(encryptdata + buf->len - 10, hash, OBFS_HMAC_SHA1_LEN)) {
            buffer_free(result); result = NULL;
            return result;
        }
...
struct buffer_t * tls12_ticket_auth_server_encode(struct obfs_t *obfs, const struct buffer_t *buf) {
...

    {
        ...
        chunk2 = buffer_alloc(SSR_BUFF_SIZE);
        ...
        data = buffer_alloc(SSR_BUFF_SIZE);
...
struct buffer_t * tls12_ticket_auth_server_decode(struct obfs_t *obfs, const struct buffer_t *buf, bool *need_decrypt, bool *need_feedback) {
    ...
    buffer_free(buf_copy);
    buffer_free(ogn_buf);
    buffer_free(verifyid);
    buffer_free(sessionid);
    return result;
}
...
struct buffer_t * tls12_ticket_fastauth_client_encode(struct obfs_t *obfs, const struct buffer_t *buf);
struct buffer_t * tls12_ticket_fastauth_client_decode(struct obfs_t *obfs, const struct buffer_t *buf, bool *needsendback);
...
struct buffer_t * tls12_ticket_fastauth_client_encode(struct obfs_t *obfs, const struct buffer_t *buf) {
    return tls12_ticket_auth_client_encode(obfs, buf);
}

struct buffer_t * tls12_ticket_fastauth_client_decode(struct obfs_t *obfs, const struct buffer_t *buf, bool *needsendback) {
    return tls12_ticket_auth_client_decode(obfs, buf, needsendback);
}
