...
static void
free_connections(struct uv_loop_s *loop)
{
...
server_resolve_cb(struct sockaddr *addr, void *data)
{
    ...
    struct uv_loop_s *loop = server->listen_ctx->loop;
...
{
    struct listen_ctx_t *listener = (struct listen_ctx_t *)w;
...
main(int argc, char **argv)
{
    ...
    const char *optstring = "f:s:p:l:k:t:m:b:c:i:d:a:n:huUvA6";
    ...
    // initialize ev loop
    struct uv_loop_s *loop = EV_DEFAULT;
    ...
    struct listen_ctx_t listen_ctx_list[server_num];
    ...
    while (server_num > 0) {
        ...
        if (mode != UDP_ONLY) {
            ...
            struct listen_ctx_t *listen_ctx = &listen_ctx_list[index];
    ...
    for (int i = 0; i <= server_num; i++) {
        struct listen_ctx_t *listen_ctx = &listen_ctx_list[i];
        if (mode != UDP_ONLY) {
